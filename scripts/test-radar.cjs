...
const historySource = fs.readFileSync('lib/sellerRouteHistory.ts', 'utf8');
const historyJs = ts.transpileModule(historySource, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } }).outputText;
const historyModule = { exports: {} };
new Function('module', 'exports', historyJs)(historyModule, historyModule.exports);
...
new Function('module', 'exports', 'require', js)(moduleObj, moduleObj.exports, (id) => {
  ...
  if (id === '@/lib/sellerRouteHistory') return historyModule.exports;
...
const unplannedSecondVisit = failed.concat([{ routeId: 5, routeName: 'Rota não planejada (5)', normalizedId: '100', rawId: 'BRP1001', idType: 'seller', sellerName: 'Seller X', preparedPackages: 80, collectedPackages: 0, estimatedPackages: 80, status: 'pending', hasProblem: false, timeFrom: 500 }]);
doc = buildRadarDocument(routes.concat([{ id: 5, routeName: 'Rota não planejada (5)', status: 'pending' }]), unplannedSecondVisit, existing);
assert(doc.items['100'].cluster === 'C12', 'visita avulsa deve preservar o cluster da rota programada anterior');
assert(doc.items['100'].clusterFromHistory === true, 'Radar deve marcar que o cluster veio do histórico');
assert(doc.items['100'].clusterSourceRouteName === 'BRRJ02_C12_01', 'Radar deve preservar a rota que originou o cluster');
console.log('Radar engine: 14 cenários aprovados.');
